...
def feedback(condition: str, lesson: Lesson, rng: random.Random, caregiver: CaregiverLanguage):
    if condition == "contingent_caregiver_rl":
        return lesson.correction, lesson.analogous_prompt, lesson.analogous_target, lesson.concept
    if condition == "yoked_caregiver_rl":
        # Match caregiver-token quantity and lesson form while breaking the link
        # between the child's error and what is taught next.
        if lesson.concept in CONCEPTS:
            other = rng.choice([c for c in CONCEPTS if c != lesson.concept])
        else:
            other = rng.choice(CONCEPTS)
        other_lesson = make_lesson(other, rng.randrange(1_000_000), rng.randrange(4), caregiver)
        return other_lesson.correction, other_lesson.analogous_prompt, other_lesson.analogous_target, other
    return "", lesson.analogous_prompt, lesson.analogous_target, lesson.concept
...
def collect_rollout(model: Student, tok: Tokenizer, condition: str, rng: random.Random, lessons: list[Lesson], caregiver: CaregiverLanguage, device):
    ...
    retry_concepts = Counter()
    for i, lesson in enumerate(lessons):
        target, action = RESPONSE_VOCAB.index(lesson.target), int(actions[i])
        reward = centered_reward(action, target)
        transitions.append(Transition(contexts[i], action, reward, float(logps[i]), float(values[i]), i))
        visible += len(tok.encode(contexts[i])) + len(tok.encode(" " + RESPONSE_VOCAB[action]))
        choices[(lesson.concept, RESPONSE_VOCAB[action])] += 1
        rewards[lesson.concept].append(reward)
        if action == target:
            attempt1 += 1
            eventual += 1
            continue
        if condition == "trial_only_rl":
            continue
        teach, retry_prompt, retry_target, retry_concept = feedback(condition, lesson, rng, caregiver)
        retry_context = contexts[i] + " " + RESPONSE_VOCAB[action] + "\n" + teach + "\n" + retry_prompt
        retry_rows.append((i, retry_context, retry_target, retry_concept))
        visible += len(tok.encode("\n" + teach + "\n" + retry_prompt))
        retries += 1
        retry_concepts[retry_concept] += 1
    if retry_rows:
        with torch.no_grad():
            ...
            ractions = rdist.sample()
            rlogps = rdist.log_prob(ractions)
        for j, (dialogue, context, target_text, retry_concept) in enumerate(retry_rows):
            target, action = RESPONSE_VOCAB.index(target_text), int(ractions[j])
            reward = 0.6 if action == target else -1.0 / (len(RESPONSE_VOCAB) - 1)
            transitions.append(Transition(context, action, reward, float(rlogps[j]), float(rvalues[j]), dialogue))
            visible += len(tok.encode(" " + RESPONSE_VOCAB[action]))
            eventual += int(action == target)
            choices[(retry_concept, RESPONSE_VOCAB[action])] += 1
            rewards[retry_concept].append(reward)
    metrics = {
        "dialogues": len(lessons),
        "transitions": len(transitions),
        "retries": retries,
        "attempt1_accuracy": attempt1 / len(lessons),
        "eventual_success": eventual / len(lessons),
        "candidate_counts": {f"{a}::{b}": n for (a, b), n in choices.items()},
        "retry_concepts": dict(retry_concepts),
        "mean_reward_by_concept": {k: float(np.mean(v)) for k, v in rewards.items()},
    }
    return transitions, visible, metrics


def assign_gae(transitions, gamma=0.99, lam=0.95):
...
def ppo_update(model, tok, optimizer, transitions, device, rng, epochs=2, minibatch=64, clip=0.2, value_coef=0.5, target_kl=0.03):
    if not transitions:
        return {"loss": 0.0, "approx_kl": 0.0, "max_minibatch_kl": 0.0, "clip_fraction": 0.0, "early_stopped_for_kl": False}
    assign_gae(transitions)
    advantages = torch.tensor([x.advantage for x in transitions], device=device)
    advantages = (advantages - advantages.mean()) / advantages.std(unbiased=False).clamp_min(1e-6)
    indices, kls, clips, losses, entropies, early = list(range(len(transitions))), [], [], [], [], False
    for _ in range(epochs):
        rng.shuffle(indices)
        for start in range(0, len(indices), minibatch):
            selected = indices[start:start + minibatch]
            batch = [transitions[i] for i in selected]
            ...
            new = dist.log_prob(actions)
            old = torch.tensor([x.old_logp for x in batch], device=device)
            returns = torch.tensor([x.return_ for x in batch], device=device)
            adv = advantages[torch.tensor(selected, device=device)]
            ratio = (new - old).exp()
            objective = torch.minimum(ratio * adv, ratio.clamp(1 - clip, 1 + clip) * adv)
            entropy = dist.entropy().mean()
            loss = -objective.mean() + value_coef * 0.5 * (values - returns).pow(2).mean() - 0.01 * entropy
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            kl = 0.5 * (new.detach() - old).pow(2).mean().item()
            kls.append(kl)
            clips.append(((ratio.detach() < 1 - clip) | (ratio.detach() > 1 + clip)).float().mean().item())
    ...
    return {
        ...
        "max_minibatch_kl": float(max(kls)),
        "clip_fraction": float(np.mean(clips)),
        "entropy": float(np.mean(entropies)),
        "optimizer_minibatches": len(kls),
        "early_stopped_for_kl": early,
    }
