...
def evaluate(model, tok, device, split, caregiver, items=128, seed=900000):
    ...
    with torch.no_grad():
        for concept in CONCEPTS:
            ...
            target = RESPONSE_VOCAB.index(concept)
            result[concept] = {
                "accuracy": sum(x == target for x in actions) / len(actions),
                "selections": dict(Counter(RESPONSE_VOCAB[x] for x in actions)),
                "mean_entropy": float(torch.distributions.Categorical(logits=scores).entropy().mean().cpu()),
                "mean_target_logit": float(scores[:, target].mean().cpu()),
            }
    model.train()
    result["macro_accuracy"] = float(np.mean([result[x]["accuracy"] for x in CONCEPTS]))
    return result
...
def _caregiver(args):
    path = Path(args.caregiver_cache) if args.caregiver_cache else None
    return CaregiverLanguage.load(str(path) if path and path.exists() else None)


def _config(args, tokenizer_path, caregiver):
    caregiver_path = Path(args.caregiver_cache) if args.caregiver_cache else None
    return {
        ...
        "clm_batch": args.clm_batch,
        "ppo_minibatch": args.ppo_minibatch,
        "target_kl": args.target_kl,
        "eval_interval": args.eval_interval,
        "eval_items": args.eval_items,
        "retention_tokens": args.retention_tokens,
        "withhold_concept": args.withhold_concept,
        "tokenizer_sha256": hashlib.sha256(tokenizer_path.read_bytes()).hexdigest(),
        "caregiver_sha256": hashlib.sha256(caregiver_path.read_bytes()).hexdigest() if caregiver_path and caregiver_path.exists() else None,
...
def run(args):
    ...
    rng, caregiver = random.Random(args.seed), _caregiver(args)
    tokenizer_path = Path(args.tokenizer)
    build_tokenizer(tokenizer_path, caregiver, args.vocab_size)
    ...
    config = _config(args, tokenizer_path, caregiver)
    run_dir = Path(args.output) / f"phase1-{args.condition}-{args.seed}"
    ...
    if args.resume:
        ...
        random.setstate(payload["global_python_rng"])
        np.random.set_state(payload["numpy_rng"])
        torch.set_rng_state(payload["torch_rng"])
        if device.type == "cuda" and payload.get("cuda_rng") is not None:
            torch.cuda.set_rng_state_all(payload["cuda_rng"])
    start = time.perf_counter()
    next_eval = ((visible // args.eval_interval) + 1) * args.eval_interval
    termination_reason = "budget_complete"
    while visible < args.budget:
        if args.max_steps and step >= args.max_steps:
            termination_reason = "max_steps"
            break
        if args.session_seconds and time.perf_counter() - start >= args.session_seconds:
            termination_reason = "session_limit"
            break
        step += 1
        progress = visible / args.budget
        if args.retention_tokens and not holdout and visible >= args.budget - args.retention_tokens:
            retention_pre = evaluate(model, tok, device, "far", caregiver, args.eval_items, 2_900_000)
            holdout = True
            atomic_json(run_dir / "retention_pre.json", retention_pre)
        ...
        if "clm" in args.condition:
            ...
            texts = [canonical_transcript(x) for x in lessons]
            loss = clm_loss(model, tok, texts, device)
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            increment = sum(len(tok.encode(x)) + 1 for x in texts)
            metrics = {
                "clm_loss": float(loss.detach()),
                "examples_by_concept": dict(Counter(x.concept for x in lessons)),
            }
        else:
        ...
        if visible >= next_eval or visible >= args.budget:
            ...
            event["far"] = evaluate(model, tok, device, "far", caregiver, args.eval_items, 2_000_000 + step * 1000)
            while next_eval <= visible:
        ...
        tmp = run_dir / "latest.pt.tmp"
        torch.save(payload, tmp)
        os.replace(tmp, run_dir / "latest.pt")
        atomic_json(run_dir / "trace.json", {"config": config, "events": events})
    far = evaluate(model, tok, device, "far", caregiver, args.eval_items, 4_000_000)
    retention = None
    if retention_pre is not None:
        chance = 1 / len(RESPONSE_VOCAB)
        pre, post = retention_pre[args.withhold_concept]["accuracy"], far[args.withhold_concept]["accuracy"]
        retention = {
            "concept": args.withhold_concept,
            "pre": pre,
            "post": post,
            "drop": pre - post,
            "chance_adjusted_ratio": None if pre <= chance else (post - chance) / (pre - chance),
        }
    final = {
        "config": config,
        "visible_tokens": visible,
        "steps": step,
        "complete": visible >= args.budget,
        "termination_reason": termination_reason,
        "near": evaluate(model, tok, device, "near", caregiver, args.eval_items, 3_000_000),
        "far": far,
        "retention": retention,
        "run_dir": str(run_dir),
    }
...
def overfit_test(args):
    ...
    build_tokenizer(path, caregiver, args.vocab_size)
    tok = Tokenizer(path)
    model = Student(tok.vocab_size, args.layers, args.hidden, args.heads, args.ff).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=args.policy_lr)
    fixed = [make_lesson("code", i, i % 4, caregiver) for i in range(1000)]
    history = []
    for rollout in range(1, args.overfit_rollouts + 1):
        ...
        update = ppo_update(
            ...
            rng,
            minibatch=args.ppo_minibatch,
            target_kl=max(args.target_kl, 0.2),
        )
        if rollout == 1 or rollout % 5 == 0:
            ...
            if accuracy >= 0.95:
                return {"passed": True, "accuracy": accuracy, "rollouts": rollout, "history": history}
